...
ETLantic 0.14.0 discovers portable transformation compilers through the
...
dependencies = ["etlantic>=0.14,<0.15", "acme-frame>=2,<3"]
...
profile. In 0.14.0, a relational claim must reject join collision policies
...
Install the wheel into an isolated environment with ETLantic 0.14.0, then:
...
Depend on a matching ETLantic minor (`etlantic>=0.14.0,<0.15` for official
0.14 plugins).
