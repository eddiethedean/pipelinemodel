...
from pipelantic.interchange.odcs import load_data_contract, odcs_filename
from pipelantic.interchange.policy import DEFAULT_DPCS_VERSION, check_dpcs_version
...
from pipelantic.pipeline import Sink, Source
from pipelantic.refs import OutputRef


class DpcsError(PipelanticError):
            ...
            interface_inputs.append(
                {
                    ...
                    "pipelantic:binding": node.binding,
                }
            )
...
def pipeline_from_dpcs(
    ...
    *,
    ...
    for entry in doc.get("interface", {}).get("inputs") or []:
        ...
        binding = entry.get("pipelantic:binding") or name
